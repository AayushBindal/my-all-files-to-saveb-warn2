...
function setup() {
  ...
  console.log(windowHeight+"this is height");
  ...
  reset.position(windowWidth-120, 50);
  let p = createP("Press Space to get a second Chance to Play!!",50 ,50);
  p.position(windowWidth/360, 0);
  p.addClass("p");
  fullScreen = createButton("✥");
  fullScreen.position(windowWidth-120, 120);
  incSpeed=createButton("💨");
  incSpeed.position(windowWidth-120, 190);
}
...
function keyPressed() {
	if (keyCode === 32) {
    Matter.Body.setPosition(stoneObj.body, {x:windowWidth/235, y:windowWidth/420}) 
